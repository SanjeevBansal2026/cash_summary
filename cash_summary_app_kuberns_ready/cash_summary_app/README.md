...
This project is prepared for Kuberns deployment without uploading a local virtual environment.
Kuberns can auto-detect Django projects from `manage.py`, `requirements.txt`, and `wsgi.py`.
A `Procfile` is also included with a Gunicorn web command.

Recommended Kuberns environment variables:
- `DJANGO_SECRET_KEY` = a new long random secret
- `DJANGO_DEBUG` = `0`
- `DJANGO_ALLOWED_HOSTS` = your Kuberns hostname/custom domain (comma separated)
- `CSRF_TRUSTED_ORIGINS` = `https://your-domain.example.com` (comma separated)

Run migrations from Kuberns Post-Build/Release commands if required:
`python manage.py migrate`

Static files:
`python manage.py collectstatic --noinput`

Do not upload `.env` or a local `venv`/`.venv` directory.
